...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        # decode
        self.Up5 = up_conv(ch_in=1037,ch_out=512)
    ...
    def decode(self, zs, a):
        a_tile = a.view(a.size(0), -1, 1, 1).repeat(1, 1, 8, 8) # 将特征向量延展为 8*8*13
        # a_tile 8*8*13
        z = torch.cat([zs[4], a_tile], dim=1)      # 特征嵌入层
        # z 8*8*1037
        ...
        # d5 16*16*512
        # x4 16*16*512
        z_a = self.Att5(g=z,x=zs[3])
        # x4 16*16*512
        z = torch.cat((z_a,z),dim=1)
        # d5 16*16*1024        
        z = self.Up_conv5(z)
        # d5 16*16*512
