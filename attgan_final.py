...
class Generator(nn.Module):
    ...
    def decode(self, zs, a):
        ...
        z = self.dec_layers[7](z)                                                   # 7(SA)
        x_d = z                                                                     # 8
        x_f = zs[3]                                                                 # 编解码器的对应层数关系
        decode_l = self.dec_layers[8]
        att_out = decode_l(g=x_d ,x=x_f)
        z = torch.cat((att_out,x_d),dim=1)
        z = self.dec_layers[9](z)                                                   # 9
        z = self.dec_layers[10](z)                                                  # 10
        z = self.dec_layers[11](z)                                                  # 11(SA)
        x_d = z                                                                     # 12
        x_f = zs[2]                                                                 # 编解码器的对应层数关系
        decode_l = self.dec_layers[12]
        att_out = decode_l(g=x_d ,x=x_f)
        z = torch.cat((att_out,x_d),dim=1)
        z = self.dec_layers[13](z)                                                   # 13
        z = self.dec_layers[14](z)                                                   # 14
        # end
        return z
