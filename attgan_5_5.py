...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        layers += [Conv2dBlock(64, 128, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)]                 # 1
        layers += [Conv2dBlock(128, 256, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)]                # 2
        layers += [Conv2dBlock(256, 512, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)]                # 3
        layers += [Conv2dBlock(512, 1024, (4, 4), stride=2, padding=1, norm_fn=enc_norm_fn, acti_fn=enc_acti_fn)]               # 4
        ...
        layers += [My_Self_Attn(512, 'relu')]                                                                                   # 1
        ...
        layers += [My_Self_Attn(256, 'relu')]                                                                                   # 4
        ...
        layers += [ConvTranspose2dBlock(128, 64, (4, 4), stride=2, padding=1, norm_fn=dec_norm_fn, acti_fn=dec_acti_fn)]        # 7
        layers += [ConvTranspose2dBlock(64, 3, (4, 4), stride=2, padding=1, norm_fn='none', acti_fn='tanh')]                    # 8
    ...
    def decode(self, zs, a):
        ...
        x_d = z                                                                     # 1
        x_f = zs[3]                                                                 # 编解码器的对应层数关系
        decode_l = self.dec_layers[1]
        att_out = decode_l(Q=x_d ,K=x_f)
...
class Self_Attn(nn.Module):
    ...
    def forward(self,x):
        ...
        proj_query  = self.query_conv(x).view(m_batchsize,-1,width*height).permute(0,2,1)               # B x (C/8) x (W*H) 将Q卷积后的特征拉长为2维的，B*(C/8)*(H*W),后经过转置 B*(H*W)*(C/8)
        proj_key =  self.key_conv(x).view(m_batchsize,-1,width*height)                                  # B x (C/8) x (W*H) 将K卷积后的特征拉长为2维的，B*(C/8)*(H*W)
...
# 互补attention结构，用于搭建 替换att-net
class My_Self_Attn(nn.Module):
    """ Self attention Layer"""
    def __init__(self,in_dim,activation):                                                               # 构造函数
        super(My_Self_Attn,self).__init__()
        self.chanel_in = in_dim                                                                         # 输入通道数
        self.activation = activation                                                                    # 父类里的属性，激活函数
    ...
    def forward(self,Q,K):
        """
            inputs :
                Q : decoder_input feature maps( B X C X W X H)      V1
                K : encoder_input feature maps( B X C X W X H)      V2
                attentionK mask
                attentionQ res mask
            returns :
                out : self attention value + input feature 
                attention: B X N X N (N is Width*Height)
        """
        m_batchsize,C,width ,height = Q.size()                                                          # X : B*C*W*H 获取B,C,W,H的维度信息
        # print('message',m_batchsize,C,width ,height)
        proj_query  = self.query_conv(Q).view(m_batchsize,-1,width*height).permute(0,2,1)               # B x (C/8) x (W*H) 将Q卷积后的特征拉长为2维的，B*(C/8)*(H*W),后经过转置 B*(H*W)*(C/8)
        proj_key =  self.key_conv(K).view(m_batchsize,-1,width*height)                                  # B x (C/8) x (W*H) 将K卷积后的特征拉长为2维的，B*(C/8)*(H*W)
        energy =  torch.bmm(proj_query,proj_key)                                                        # transpose check  B*(H*W)*(H*W)

        attentionK = self.softmax(energy)                                                               # B x (N) x (N)     特征mask
        attentionQ = 1-self.softmax(energy)                                                             # B x (N) x (N)     特征mask

        proj_value1 = self.value_conv(Q).view(m_batchsize,-1,width*height)                              # B x C x N 将V1卷积后的特征拉长为2维的，B*C*(H*W)
        proj_value2 = self.value_conv(K).view(m_batchsize,-1,width*height)                              # B x C x N 将V2卷积后的特征拉长为2维的，B*C*(H*W)

        out2 = torch.bmm(proj_value2,attentionK.permute(0,2,1))                                         # 将V与attention_map相乘 (B*C*N)(B*N*N) = B*C*N
        out1 = torch.bmm(proj_value1,attentionQ.permute(0,2,1))                                         # 将V与attention_map相乘 (B*C*N)(B*N*N) = B*C*N

        out2 = out2.view(m_batchsize,C,width,height)                                                    # 再还原成原图像 B*C*H*W
        out1 = out1.view(m_batchsize,C,width,height)                                                    # 再还原成原图像 B*C*H*W
        out = out1 + out2
