...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        for i in range(dec_layers):
            if i < dec_layers - 1:
                ...
                layers += [Attention_block(F_g=n_out,F_l=n_out//2,F_int=n_out//2)]      # attention模块输入F_g为输入的一半
    ...
    def decode(self, zs, a):
        a_tile = a.view(a.size(0), -1, 1, 1).repeat(1, 1, self.f_size, self.f_size) # 将特征向量延展为 8*8*13
...
class AttGAN():
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_):
        ...
        for i in range(100):
            print(dc_fake.size(),dc_fake.dtype)
            print(att_b.size(),att_b.dtype)
